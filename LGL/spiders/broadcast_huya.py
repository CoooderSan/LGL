...
def get_broadcast_room(game):
    ...
    # 获取房间列表节点
    room_node_list = soup.select('#js-live-list li')
...
def get_room_info(game):
    ...
    for li in room_node_list:
        ...
        # 获取房间名称
        room_info['title'] = li.find('a', class_='title new-clickstat').attrs['title']
        # 获取播主名字
        room_info['broadcaster'] = li.find('i', class_='nick').string
        # 获取观看人数
        viewers = li.find('i', class_='js-num').string
        # 如果观看人数过万则转化为纯数字格式
        # print(viewers[-1] == "万")
        if viewers[-1] == "万":
            viewers = viewers[:-1]
        ...
        # 获取直播图路径
        room_info['img'] = 'https://'+li.find('img').attrs['data-original'][2:]
        # 获取房间路径
        room_info['roomUrl'] = li.find('a', class_='video-info new-clickstat').attrs['href']
        # print(room_info)
        # 将单个房间添加到房间列表中
        if viewers >= 10000:
